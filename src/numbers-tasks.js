...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const vectorsScalarProduct = x1 * x2 + y1 * y2;
  const firstVectorLength = Math.sqrt(x1 * x1 + y1 * y1);
  const secondVecotorLength = Math.sqrt(x2 * x2 + y2 * y2);
  let angleCos =
    vectorsScalarProduct / (firstVectorLength * secondVecotorLength);
  angleCos = Math.max(-1, Math.min(1, angleCos));
  const angleInRad = Math.acos(angleCos);
  return angleInRad;
}
...
function isPrime(n) {
  ...
  const squaredNum = Math.sqrt(n);
  for (let i = 3; i <= squaredNum; i += 2) {
...
function getRandomInteger(min, max) {
  ...
  const randomInt = Math.floor(Math.random() * (maxF - minC) + minC);
  return randomInt;
}
...
function getCountOfOddNumbers(number) {
  return Array.from({ length: Math.abs(number) }, (_, index) => index).reduce(
    (prev, curr) => (curr % 2 === 0 ? prev + 1 : prev + 0),
    0
  );
}
